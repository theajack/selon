...
function initApiFunBtn(){
  
  J.id("apiCodeBtn").clk(showResult).tip("Submit");
  J.id("apiCodeResetBtn").clk(function(){
    J.confirm("Are you sure to reset code,you will lose all code you are editting!",resetApiCode);
  }).tip("Reset code");
...
function initApiCodeMain(){
  J.id("apiCode").on({
    mouseleave:function(){
      showResult(false);
    },
    keydown:codeApiChange,
    keyup:function(e){
...
function geneApiViewCode(){
  //moveCursor();.replaceAll("<","&lt;").replaceAll(">","&gt;")
  var html=J.id("apiCode").val();
  html=html.replaceAll("<","&lt;").replaceAll(">","&gt;");
  
  var script=html.substring(html.indexOf("&lt;script"),html.indexOf("&lt;\/script&gt;")+"&lt;\/script&gt;".length);
  var elems=html.replace(script,"");
  
  
  script=geneApiSign(script);
  script=geneApiKey(script);
  script=geneApiHtmlElement(script);
  elems=geneApiHtmlElement(elems);
  elems=geneApiHtmlNote(elems);
  
  
  script=geneApiString(script);
  script=geneApiNote(script);
  J.id("apiCodeView").html(elems+script);
  if(J.id("apiCode").attr("show-all")=="true"){
    resetApiAllCode();
  }
}
  function geneApiHtmlElement(html){
  ...
  function geneApiSign(html){
    if(html.has("script")){
      ...
      script=geneApiFun(script);
      script=geneApiDefineFun(script);
      script=geneApiNumber(script);
      html=elems.replace("$R$",script);
      return html;
    }
    return html;
  }function geneApiNumber(html){
  ...
  function geneApiDefineFun(html){
